function [TckResultCT_pos, navSolutionsCT] = trackingCT_POS_KF(file,signal,track,cmn, Acquired,TckResult_Eph,cnslxyz,eph,sbf,solu)
%Purpose:
...
state_cov = 1*diag([1e-1,1e-1,1e-1,1e-1,1e-1,1e-1,1e0,1e0]);

% process (System) noise noise covariance matrix
process_noise(1:3,1:3) = diag(ones(1,3)*2e-5);
process_noise(4:6,4:6) = diag(ones(1,3)*1e-5);
process_noise(7,7) = 1e-5;
process_noise(8,8) = 1e-6;
...
for msIndex = 1:100
    ...
    for svindex = 1 :svlength
        ...
        % Data Recording
        TckResultCT_pos(prn).P_i(msIndex)             = P_i;
        TckResultCT_pos(prn).P_q(msIndex)             = P_q;
        TckResultCT_pos(prn).E_i(msIndex)             = E_i;
        TckResultCT_pos(prn).E_q(msIndex)             = E_q;
        TckResultCT_pos(prn).L_i(msIndex)             = L_i;
        TckResultCT_pos(prn).L_q(msIndex)             = L_q;
        TckResultCT_pos(prn).carrError(msIndex)       = carrError(svindex);
        TckResultCT_pos(prn).codeError(msIndex)       = codeError(svindex);
        TckResultCT_pos(prn).codeFreq(msIndex)        = codeFreq(svindex);
        TckResultCT_pos(prn).carrFreq(msIndex)        = carrFreq(svindex);
        TckResultCT_pos(prn).numSample(msIndex)       = numSample;
        TckResultCT_pos(prn).remChip(msIndex)         = remChip(svindex);        
        TckResultCT_pos(prn).remCarrPhase(msIndex)    = remCarrPhase(svindex);
        TckResultCT_pos(prn).absoluteSample(msIndex)  = ftell(file.fid); 
        TckResultCT_pos(prn).absoluteSampleCodedelay(msIndex)  = mod(TckResultCT_pos(prn).absoluteSample(msIndex)/...
                                                                (file.dataPrecision*file.dataType),fs*t);
        TckResultCT_pos(prn).codedelay(msIndex)       = mod(TckResultCT_pos(prn).absoluteSample(msIndex)/ ...
                                                        (file.dataPrecision*file.dataType),fs*t);
        TckResultCT_pos(prn).delayValue(msIndex)      = delayValue(svindex,msIndex);
    ...
    for svindex=1:svlength
        ...
        x1 = [oldabsoluteSample_pos(svindex)/(file.dataPrecision*file.dataType) ...
                TckResultCT_pos(prn).absoluteSample(msIndex)/(file.dataPrecision*file.dataType)];
      	y2 = [oldcodedelay_pos(svindex) TckResultCT_pos(prn).codedelay(msIndex)];
    ...
    oldabsoluteSample_pos(svindex) = TckResultCT_pos(prn).absoluteSample(msIndex);
    
   
    %% Pseudorange calculation and correction     
    [pseudorange, ~] = pr_est(Acquired, sbf, signal,TckResultCT_pos, msIndex);
    ...
    for svindex = 1 : svlength
        ...
        d_p(svindex,1)          = (TckResultCT_pos(Acquired.sv(svindex)).carrFreq(msIndex) ...
                                        - signal.IF)*cmn.cSpeed/fL1 + sum( sv_vel(svindex,:) .* a(svindex,:));
 
        % measured pseudorange rate
        prr_measured(svindex)	= (TckResultCT_pos(Acquired.sv(svindex)).carrFreq(msIndex) - signal.IF)*cmn.cSpeed/fL1 ;
        ...
        pr_delta(svindex,1) = (codeError(svindex) - 0)*cmn.cSpeed/codeFreq(svindex);
    end
    ...
    if flag_corrCovEst2 == 1 && counterUptR == thresUptR
        ...
        for idx = 1 : svlength
            if mesurement_noise(idx,idx) >= 100000
                mesurement_noise(idx,idx) = 100000;
            elseif mesurement_noise(idx,idx) <= 1 	
                mesurement_noise(idx,idx) = 1;	
            end
            if mesurement_noise(idx+svlength,idx+svlength) >= 100000
                mesurement_noise(idx+svlength,idx+svlength) = 100000;
            elseif mesurement_noise(idx+svlength,idx+svlength) <= 1 
                mesurement_noise(idx+svlength,idx+svlength) = 1;	
            end
        end
        counterUptR = 0;
    ...
    fprintf('WLS: index = %4d; localTime: %f;  X = %f Y = %f Z = %f  VE = %f VN = %f VU = %f\n\n', ...
        msIndex, TOW_USR_CT(msIndex), usrenu(msIndex,1),usrenu(msIndex,2),usrenu(msIndex,3),usrenuvel(msIndex,1), usrenuvel(msIndex,2), usrenuvel(msIndex,3));
...
save(['navSolCT_KF_',file.fileName], 'navSolutionsCT','eph','TOW_USR_CT');
save(['tckRstCT_KF_',file.fileName], 'TckResultCT_pos','CN0_CT'); 
